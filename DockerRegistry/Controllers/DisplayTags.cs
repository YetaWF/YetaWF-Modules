...
namespace YetaWF.Modules.DockerRegistry.Controllers {
    ...
    public class DisplayTagsModuleController : ControllerImpl<YetaWF.Modules.DockerRegistry.Modules.DisplayTagsModule> {
        ...
        public class BrowseItem {
            ...
            public MenuList Commands {
                get {
                    ...
                    actions.New(Module.GetAction_RemoveTag(RegistryId, Registry, Digest));
                    return actions;
                }
            }
            ...
            public BrowseItem(DockerTagEntry data, DisplayTagsModule module, int registryId, string registry) {
                Module = module;
        ...
        private GridDefinition GetGridModel(int registryId, string registry) {
            return new GridDefinition {
                ...
                ExtraData = new BrowseItem.ExtraData { RegistryId = registryId, Registry = registry },
                ...
                DirectDataAsync = async (int skip, int take, List<DataProviderSortInfo> sort, List<DataProviderFilterInfo> filters) => {
                    using (RegistryEntryDataProvider regDP = new RegistryEntryDataProvider()) {
                        ...
                        using (DockerRegistryDataProvider dockerRegDP = new DockerRegistryDataProvider()) {
                            DataProviderGetRecords<DockerTagEntry> browseItems = await dockerRegDP.GetTagsAsync(reg.RegistryURL, registry, reg.UserName, reg.Password, sort, filters);
                            return new DataSourceResult {
                                Data = (from s in browseItems.Data select new BrowseItem(s, Module, registryId, registry)).ToList<object>(),
        ...
        [AllowGet]
        public async Task<ActionResult> DisplayTags(int registryId, string registry) {
            using (RegistryEntryDataProvider regDP = new RegistryEntryDataProvider()) {
                RegistryEntry reg = await regDP.GetItemAsync(registryId);
                if (reg == null)
                    throw new Error(this.__ResStr("gone", "The registry with id {0} has been removed.", registryId));
                BrowseModel model = new BrowseModel {
                    GridDef = GetGridModel(registryId, registry)
                };
                return View(model);
            }
        }
        ...
        public async Task<ActionResult> TemplateGrid_SortFilter(GridPartialViewData gridPVData, int registryId, string registry) {
            return await GridPartialViewAsync<BrowseItem>(GetGridModel(registryId, registry), gridPVData);
        }

        [AllowPost]
        [Permission("RemoveTags")]
        [ExcludeDemoMode]
        public async Task<ActionResult> Remove(int registryId, string registry, string reference) {
            using (RegistryEntryDataProvider regDP = new RegistryEntryDataProvider()) {
                RegistryEntry reg = await regDP.GetItemAsync(registryId);
                if (reg == null)
                    throw new Error(this.__ResStr("alreadyDeleted", "The registry with id {0} has already been removed.", registryId));
                using (DockerRegistryDataProvider dockerRegDP = new DockerRegistryDataProvider()) {
                    await dockerRegDP.RemoveTagAsync(reg.RegistryURL, registry, reference, reg.UserName, reg.Password);
