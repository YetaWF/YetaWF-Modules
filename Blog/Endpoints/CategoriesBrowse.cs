...
public class CategoriesBrowseModuleEndpoints : YetaWFEndpoints {
    ...
    public static void RegisterEndpoints(IEndpointRouteBuilder endpoints, Package package, string areaName) {
        ...
        RouteGroupBuilder group = endpoints.MapGroup(GetPackageApiRoute(package, typeof(CategoriesBrowseModuleEndpoints)))
            .RequireAuthorization()
            .AntiForgeryToken();

        group.MapPost(GridSupport.BrowseGridData, async (HttpContext context, [FromBody] GridSupport.GridPartialViewData gridPvData) => {
            CategoriesBrowseModule module = await GetModuleAsync<CategoriesBrowseModule>(gridPvData.__ModuleGuid);
