...
public class CommentAddModule : ModuleDefinition {
    ...
    public async Task<ActionInfo> RenderModuleAsync(int blogEntry) {
        ...
        using (BlogEntryDataProvider entryDP = new BlogEntryDataProvider()) {
            BlogEntry? data = null;
            if (entry != 0)
                data = await entryDP.GetItemAsync(entry);
            if (data == null)
        ...
        using (BlogEntryDataProvider entryDP = new BlogEntryDataProvider()) {
            ...
            using (BlogCommentDataProvider blogCommentDP = new BlogCommentDataProvider(model.EntryIdentity)) {
                ...
                if (config.NotifyNewComment) {
                    ...
                    await sendEmail.PrepareEmailMessageAsync(config.NotifyEmail, subject, await sendEmail.GetEmailFileAsync(AreaRegistration.CurrentPackage, "New Comment.txt"), Parameters: parms);
                    await sendEmail.SendAsync(true);
                }

                if (!blogComment.Approved)
                    return await FormProcessedAsync(model, this.__ResStr("okSavedReview", "New comment saved - It will be reviewed before becoming publicly viewable"), OnClose: OnCloseEnum.ReloadPage, OnPopupClose: OnPopupCloseEnum.ReloadParentPage);
                else
                    return await FormProcessedAsync(model, this.__ResStr("okSaved", "New comment added"), OnClose: OnCloseEnum.ReloadPage, OnPopupClose: OnPopupCloseEnum.ReloadParentPage);
            }
        }
    }
}
