...
public class CallEndpoints : YetaWFEndpoints {
    ...
    private static string __ResStr(string name, string defaultValue, params object?[] parms) { return ResourceAccess.GetResourceString(typeof(CallEndpoints), name, defaultValue, parms); }

    public static void RegisterEndpoints(IEndpointRouteBuilder endpoints, Package package, string areaName) {

        RouteGroupBuilder group = endpoints.MapGroup(GetPackageApiRoute(package, typeof(CallEndpoints)));

        group.MapPost(Process, async (HttpContext context, string request, string extension, int errCount, string token) => {
                ...
                foreach (ExtensionPhoneNumber notifyNumber in ivrConfig.NotificationNumbers) {
                    if (notifyNumber.SendSMS) {
                        ...
                        await sendSMS.SendMessageAsync(notifyNumber.PhoneNumber,
                            __ResStr("notifySMS", "Incoming call received from {0} ({1}, {2}, {3}, {4}) - {5}",
    ...
    private static IResult RejectResult(string reason) {
        string xmlString = @"<Response><Reject reason=""busy"" /></Response>";
        Logging.AddErrorLog($"{nameof(RejectResult)}: {reason} {xmlString}");
        return Results.Content(xmlString, "text/xml");
    }

    private static async Task<IResult> RunEntryAsync(IVRConfig ivrConfig, ScriptData script, string called, string extension, ScriptEntry entry, int errCount) {
        ...
        string actionUrl = Utility.UrlFor(typeof(CallEndpoints), nameof(Process));
#if DEBUG
                        ...
                        foreach (ExtensionNumber extNumber in ext.Numbers) {
                            if (extNumber.SendSMSVoiceMail) {
                                ...
                                await sendSMS.SendMessageAsync(extNumber.Number,
                                    __ResStr("voiceSMS", "A voice mail was received for extension {0} ({1}) from {2}, {3}, {4}, {5} {6} - {7}",
        ...
        return Results.Content(text, "text/xml");
    }

    private static int ConvertToInt(string duration) {
        try {
    ...
    private static string RepeatableNumbers(Extension ext, string text) {
    ...
    static Regex reRepeatableNumbers = new Regex(@"REPEATABLENUMBERS.*?$", RegexOptions.Multiline | RegexOptions.Compiled);

    private static string Spaced(string phoneNumber) {
    ...
    private static bool TryGetForm(string name, out string value) {
    ...
    private static string GetForm(string name) {
        string value;
    ...
    private static void LogCall(string action, string extension) {
