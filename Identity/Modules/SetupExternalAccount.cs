...
public class SetupExternalAccountModule : ModuleDefinition2 {
    ...
    [ExcludeDemoMode]
    public async Task<IResult> UpdateModuleAsync(SetupExternalAccountModel model) {
        ...
        if (user.UserStatus == UserStatusEnum.NeedValidation) {
            ...
            return await FormProcessedAsync(model, this.__ResStr("okAwaitRegistration", "An email has just been sent to your email address \"{0}\" to complete the registration. Allow a few minutes for delivery. Once received, please use the information in the email to complete the registration.", model.Email),
