    ...
    public class BackupsModule : ModuleDefinition {
        ...
        public ModuleAction GetAction_MakeSiteTemplateData() {
            ...
            return new ModuleAction(this) {
                ...
                Tooltip = this.__ResStr("makeTemplateTT", "Create a site template for installation of a new YetaWF site using the current site"),
                Legend = this.__ResStr("makeTemplateLegend", "Creates a site template for installation of a new YetaWF site using the current site"),
                Category = ModuleAction.ActionCategoryEnum.Significant,
                Mode = ModuleAction.ActionModeEnum.Any,
                Location = ModuleAction.ActionLocationEnum.ModuleLinks,
        ...
        public ModuleAction GetAction_RemoveLink(string filename) {
            if (!IsAuthorized("Backups")) return null;
            return new ModuleAction(this) {
                Url = YetaWFManager.UrlFor(typeof(BackupsModuleController), "Remove"),
                QueryArgs = new { FileName = filename },
                NeedsModuleContext = true,
                Image = "#Remove",
                Style = ModuleAction.ActionStyleEnum.Post,
