...
public class SessionInfoModuleEndpoints : YetaWFEndpoints {
    ...
    public static void RegisterEndpoints(IEndpointRouteBuilder endpoints, Package package, string areaName) {
        ...
        RouteGroupBuilder group = endpoints.MapGroup(GetPackageApiRoute(package, typeof(SessionInfoModuleEndpoints)))
            .RequireAuthorization()
            .AntiForgeryToken();

        group.MapPost(GridSupport.BrowseGridData, async (HttpContext context, [FromBody] GridSupport.GridPartialViewData gridPvData) => {
            SessionInfoModule module = await GetModuleAsync<SessionInfoModule>(gridPvData.__ModuleGuid);
