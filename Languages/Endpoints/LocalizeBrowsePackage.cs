    ...
    public class LocalizeBrowsePackageModuleEndpoints : YetaWFEndpoints {
        ...
        public static void RegisterEndpoints(IEndpointRouteBuilder endpoints, Package package, string areaName) {
            ...
            group.MapPost(GridSupport.BrowseGridData, async (HttpContext context, [FromBody] GridSupport.GridPartialViewData gridPvData, string packageName) => {
                ...
                return await GridSupport.GetGridPartialAsync(context, module, LocalizeBrowsePackageModuleController.GetGridModel(module, package), gridPvData);
            });
        ...
        private static async Task TranslatePackageAsync(string packageName, string language, Localization.Location resourceType) {
            ...
            List<LocalizeBrowsePackageModuleController.LocalizeFile> files = (from s in await Localization.GetFilesAsync(package, MultiString.DefaultLanguage, false) select new LocalizeBrowsePackageModuleController.LocalizeFile { FileName = Path.GetFileName(s) }).ToList();

            // Extract all strings into a list
            List<string> strings = new List<string>();
            List<LocalizationData> allData = new List<LocalizationData>();
            foreach (LocalizeBrowsePackageModuleController.LocalizeFile file in files) {
